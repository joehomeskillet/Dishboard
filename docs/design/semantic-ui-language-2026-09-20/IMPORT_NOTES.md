...
Die zwölf Dateien dieses Ordners sind unverändert aus dem gelieferten Paket `dishboard_icon_pack` übernommen. Laut
...
- Ein Modell: `semantic_key → Icon → Label-Key → Tooltip-Key → Aria-Key → erlaubte Darstellung → Rolle`.
  184 Bedeutungen in 15 Kategorien (`05_SEMANTIC_REGISTRY.csv`/`.json`), je drei Texte in `locale_de.json` und
  `locale_en.json` (552 Schlüssel, vollständig und symmetrisch — geprüft).
...
- Kanonische Verben: Anlegen, Hinzufügen, Bearbeiten, Öffnen, Speichern, Bestätigen, Löschen, Archivieren, Kopieren,
  Vorschau — keine Synonyme je Modul.
