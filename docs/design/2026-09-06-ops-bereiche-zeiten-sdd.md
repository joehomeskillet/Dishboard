...
| `validate_menu_item_price` | Kosten sind ausschliesslich für `staff_guest`/`LUNCH` zulässig, einschliesslich gespeicherter Wochenendservices. Der Serviceschreiber prüft die Wochenendfreigabe bei Neuanlage; nachträgliches Abschalten entwertet keine vorhandenen Menüs oder Preise. CHF-/Rappen-Constraints bleiben unverändert. |
...
- Verbraucher (Public, Signage, HTML-Druck): `snapshot.area_name` ersetzt die festen Bereichstitel («Cafeteria · Mitarbeitende und Externe», «Speiseplan für Patientinnen und Patienten» usw.); ohne Schlüssel (Schema 1) bleibt der bisherige Text. Zeiten: `service_start`/`service_end` → «Ausgabe ab 11:30 Uhr» / «Ausgabe 11:30–12:30 Uhr» (Patienten) bzw. «Mittag 11:30–13:30 Uhr» (Cafeteria); ohne Zeiten keine Zeitzeile. **Einzige Ausnahme:** Patientenausgaben aus Snapshots **ohne `area_name`** (alle vor OPS erzeugten Snapshots, unabhängig von `schema_version`; auch `tools/demo_snapshots.py` erzeugte bereits `schema_version 2` ohne OPS-Schlüssel) zeigen weiterhin die bisherigen festen Texte `Ausgabe ab 11:30/17:30 Uhr` an genau den Stellen, an denen sie heute stehen (Tagesansichten), über die Konstante `LEGACY_PATIENT_MEAL_TIMES` an genau einer Stelle; keine neuen Zeitzeilen an Stellen, die heute keine haben. Geschlossene Services zeigen nie eine Zeit. `build_snapshot` liefert `area_name` immer, deshalb ist dessen Fehlen der eindeutige Marker für Alt-Snapshots.
