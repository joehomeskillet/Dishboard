...
Stand: 13. September 2026. Paket `MP-OFF-FIELDMAP` (`wp-645d9788f698`), Review-Korrektur `wp-7194bc889111`, Semantik-Korrektur `wp-5f2ec1902690`.
...
| Produktbasis | [schemas/product_base.yaml](https://github.com/openfoodfacts/openfoodfacts-server/blob/5a203f69738a1615d160051a661a36412d6ff252/docs/api/ref/schemas/product_base.yaml) | `code` (Z. 17), `product_name` (Z. 10), `generic_name` (Z. 46), `lc`/`lang` (Z. 55/60), `product_quantity` (Z. 81), `product_quantity_unit` (Z. 87), `quantity` (Z. 95), `schema_version` (Z. 102). |
...
- **Feldselektion (`fields`)**, Namen gemäss Schema-Eigenschaften: `code,schema_version,lang,product_name,generic_name,quantity,product_quantity,product_quantity_unit,serving_size,serving_quantity,ingredients_text,allergens_tags,traces_tags,origins,origins_tags,countries_tags,nutrition,selected_images`. Zusätzlich `lc=de` für sprachabhängige Auswahl. `product_quantity_unit` (g oder ml) ist vom normalisierten Zahlenfeld `product_quantity` getrennt und darf nicht aus der Zahl allein abgeleitet werden. Ob `fields` die zur Laufzeit erzeugten `selected_images` liefert, wurde nicht live geprüft; `MP-OFF-FETCH` belegt das mit echten Request-Receipts.
...
| **Packungsmenge** | `quantity` (product_base.yaml:95), `product_quantity` (product_base.yaml:81), `product_quantity_unit` (product_base.yaml:87) | `string` (wie auf Packung), `string` (normalisierte Grösse), `string` (`g` oder `ml`) | Vorgeschlagen: Provenienz; kein automatisches Mapping auf `piece_weight_g`. | `product_quantity` ist die normalisierte Grösse (z. B. `"120"` aus `"2 x 60 g"`); `product_quantity_unit` liefert die Einheit (`g` vs. `ml`) und wird aus `quantity` berechnet (OFF-offiziell). Die Flüssigkeitsfrage bei Nährwerten folgt aus `product_quantity_unit`, nicht aus der Zahl in `product_quantity` allein. | `piece_weight_g` bleibt manuelle Entscheidung. |
...
| **Nährwerte v3.5+: Eingabesätze** | `nutrition.input_sets[]` mit `source`, `per`, `per_quantity`, `per_unit`, `preparation`, `unspecified_nutrients[]`, `last_updated_t` | `array[object]` | Vorgeschlagen: Provenienz im NUT-Vertrag. | `unspecified_nutrients` gilt **pro Eingabesatz** (`source`, z. B. `packaging`): «typischerweise vorhanden, für dieses Produkt in diesem Satz nicht angegeben». Das ist Provenienz des jeweiligen `input_set`, keine globale NULL-Anweisung für `aggregated_set`. Von «Schlüssel fehlt» zu unterscheiden. `per = serving` ohne `per_quantity`/`per_unit` ist nicht in 100 g/100 ml umrechenbar. | Nur Provenienz, keine Zielspalten. |
...
2. **Nährwerte: Wertkategorien** (gilt für v3.5+ `aggregated_set` und Legacy `_100g`):
   ...
   - **Auf der Packung nicht angegeben (pro Eingabesatz)**: v3.5+ `input_sets[].unspecified_nutrients` bezeichnet Nährstoffe, die im **jeweiligen Eingabesatz** (z. B. `source = packaging`) auf der Packung nicht angegeben sind. Das ist Provenienz dieses Satzes, keine globale Anweisung, `aggregated_set` auf `NULL` zu setzen: OFF kann dort trotzdem normalisierte Werte aus höherpriorisierten Quellen (z. B. `manufacturer`) führen; vorhandene Werte in `aggregated_set` werden aufgrund von `unspecified_nutrients` eines anderen Satzes nicht verworfen. Fehlende/unbekannte Werte bleiben `NULL`, nie `0`. Legacy `no_nutrition_data = "on"` ist eine Packungsaussage ohne rechtliche Befreiung; die frühere Deutung «exempt» ist gestrichen.
   - **Widerspruch**: Liegen trotz `no_nutrition_data = "on"` oder `unspecified_nutrients` in einem Eingabesatz normalisierte Werte in `aggregated_set` vor, werden diese nicht verworfen und nicht bestätigt, sondern als Vorschlag mit Konfliktkennzeichen geführt.
...
### Korrekturen aus dem Reviewbefund `wp-5f2ec1902690`

1. Packungsmenge: `product_quantity_unit` (product_base.yaml:87) in `fields` und Feldmatrix ergänzt; `product_quantity` (normalisierte Grösse) von `product_quantity_unit` (`g`/`ml`) getrennt; Einheit nicht aus der Zahl allein ableiten.
2. Nährwerte: `input_sets[].unspecified_nutrients` nur als Provenienz des jeweiligen Eingabesatzes; kein globales NULL für `aggregated_set`; vorhandene normalisierte Werte (z. B. aus `manufacturer`) nicht verwerfen; unbekannte Werte bleiben `NULL`.
