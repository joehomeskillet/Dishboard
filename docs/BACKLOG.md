...
|---|---|---|
| BRD-001 | **Design & Marke** | Nach Nutzerkorrektur vorgezogen: Darstellung bietet bisher nur Dichte. Zusätzliche wirksame Einstellungen für Lesbarkeit, Inhaltsbreite und Menübilder umsetzen; gleiche Kartengrössen und 48-px-Touchziele bleiben verbindlich. Logo, Farbpalette und Typografie zentral pflegen; Vorschau für Admin, Web, Screen und Druck, mit sicherem Upload und rücksetzbarer Aktivierung. |
| TPL-002 | **Visueller PDF-Druckvorlageneditor** | Nach Nutzerkorrektur vorgezogen: `/admin/vorlagen` ist bisher nur ein Navigationshub. Gesamte Druckgestaltung bearbeiten: Logo, Farben, Schrift, Abstände, Spalten, Kopf/Fuss, Bilder, Symbole, Legenden und Datenfelder. pdfme als bedingter Prototyp-Kandidat: vollständige Tabler-Bedienoberfläche erforderlich, noch keine Rendererentscheidung. Vorlagen speichern, kopieren, Vorschau, aktivieren und zurücksetzen. Cursor/Grok-4.6-Versuche blieben durch Werkzeugfreigaben unvollständig; separate Codex-Recherche liegt vor. Ein nutzbarer erster Eigenschaftseditor ersetzt nicht den vollständigen Umfang. |
| API-001 | REST-API v1, MCP, FHIR R5 und API-Schlüssel | Integration ausgehend von `37d00b7`; Backup-Fix `05b7985` mit Paketnachweis über 58 Tests und unabhängigem Teilgate über 11 Tests geprüft. Gate 4 endete mit 279 Fails, 56 Setup-Fehlern, 3070 bestandenen Tests und 15 Skips; gemeinsame Ursache ist unvollständiger beziehungsweise zu später Admin-Blueprint-Aufbau. Fix `d70460c` liegt vor; Gate 5 läuft auf `test-api-int2`. Restprüfungen, Integration und API-Deploy bleiben offen; Ablauf unten. |
...
**Status:** Seit Nutzerentscheid vom 06.09.2026, 01:55 Uhr übernimmt Codex allein. Übergabestand: `integrate/api-mcp-fhir-0905` bei `37d00b7`, Welle 1 und Welle 2 integriert, `main` bei `0ebea64` übernommen. Backup-Fix `05b7985` ist durch Paketnachweis über 58 Tests, unabhängiges Teilgate über 11 Tests und Secret-Scan geprüft. Gate 4 ist mit `GATE_EXIT=1` abgeschlossen; sein Log bleibt `.claude/state/api-mcp-fhir-0905/logs/integration-full-4.log`. Der Blueprint-Fix `d70460c` wird in Gate 5 auf `test-api-int2` geprüft, Log `/tmp/menuplan-api-bootstrap-integration-full-5-0906.log`; diesen laufenden Test weder doppeln noch seinen Worktree verändern. Lauf 3 ist obsolet. Produktion bleibt bis zum geprüften API-Deploy auf Schema 16.
