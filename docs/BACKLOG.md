...
## Produktiv, abschliessende Abnahme offen
...
Vorhandener Schlussnachweis `/tmp/dishboard-settings-filters-live-final-0906/proof.json`: **`incomplete`**, 1'090 Checks, 0 Fehler, 6 mangels passender aktiver Echtdaten nicht verfügbare positive Label-Filter-Prüfungen. Dieser Beleg ist kein vollständiges Abnahme-PASS; fehlende Datenfälle bleiben offen.
...
**Zuständigkeit seit 06.09.2026, 01:40 Uhr:** Laut `KOORDINATION-API-CODEX-2026-09-05.md` übernimmt Claude API-001 und die vollständigen Public-Screens-Pakete WP-1 bis WP-5 aus AGYs `UEBERGABE-CODEX-PUBLIC-SCREENS-2026-09-06.md`. Codex bearbeitet Symbolbasis und Admin-Backlog sowie unabhängige Gates, Integration und Deployments. Codex startet keine parallelen Public-Screens-Pakete und verändert Claudes API-/Public-Screens-Worktrees nicht.

**AGY-Anforderungen für SCR-002, ICO-001/002 und QA-001:** Hero-Food, Duo-Board und Wochenraster; Menüfotos mit Fallback, Live-Uhr, mobile Tabler-Karten und Wochentag-Pills. Flackerfreies Polling muss auch Datumswechsel bei gleicher Revision und zurückgezogene Veröffentlichungen (404) behandeln. Die vorgeschlagene unbegrenzte Offline-Anzeige wird nicht übernommen: gültige Daten, begrenztes Ausfallverhalten und Rücknahme müssen geprüft sein. Bestehende URLs, Query-Schutz, CSP und vollständige Patientenpreisfreiheit bleiben verbindlich.
...
| SCR-001 | Ein gemeinsamer Adminpunkt **Screens** | Navigation und zentrale Übersicht bei Codex in Arbeit, noch nicht geliefert. Öffentliche Ansichten und die vier vorhandenen Signage-Ziele für Cafeteria und Patienten zusammenführen; Links und aktive Zuordnung zentral verwalten. |
| SCR-002 | Bestehendes Framework für öffentliche Seiten und Bildschirme | Claude bearbeitet AGYs WP-1 bis WP-5. Vorhandenes Tabler/Bootstrap für Hero-Food, Duo-Board, Wochenraster und mobile Ansichten wiederverwenden; lesbare Web-/TV-Layouts, Fotos, Live-Uhr und flackerfreier Wechsel. Zielbrowser prüfen, keine zweite Bootstrap-Laufzeit laden; noch keine Lieferung behauptet. |
...
| TPL-001 | Neuer Adminpunkt **Vorlagen** | Navigation und zentrale Übersicht bei Codex in Arbeit, noch nicht geliefert. Zentrale Bearbeitungsübersicht für Druckvorlagen, Screen-Vorlagen, Menüvorlagen sowie Zutaten/Komponenten und Menüs. Bestehende Fachkataloge und Editoren verknüpfen, keine doppelten Datenbestände. |
...
| ICO-001 | Allergen-Symbole und Herkunftsflaggen in Admin, Web, Signage und PDF | Symbolbasis auf `integrate/backlog-symbols-0906` bei `7458cb1` vorbereitet, noch nicht produktiv. Erudus für 14 Allergengruppen, flag-icons für Länder; lokale, gepinnte SVGs, Lizenzen und gemeinsame Code-Zuordnung. Gemeinsame Verbraucher mit Claude abstimmen, PDF-Anbindung sowie Erkennbarkeit im Druck, in responsiven Browseransichten und auf Yodeck noch prüfen. |
...
| API-001 | REST-API v1, MCP, FHIR R5 und API-Schlüssel | Claude entwickelt parallel. Codex übernimmt unabhängige Gates, Integration und API-Deploy. Recovery-Commits vorbereitet; Ablauf unten. |

**Lieferfolge:** UI-001/UI-002 sind ausgeliefert; CAT-001/DSP-001 sind produktiv mit offener Schlussabnahme. Codex führt Screens-/Vorlagen-Navigation und Symbolbasis weiter; Claude liefert API-001 und Public Screens. Branding, Bereiche/Zeiten, Benutzer/Entra sowie Rezept-/Lager-/Kalkulationsfunktionen folgen dem verlinkten Ausführungsplan in getrennten geprüften Paketen. Schemaänderungen mit Claudes reservierter API-Migration 16→17 und gemeinsame Deploys abstimmen.
...
**Status:** Nutzerentscheid vom 06.09.2026: Claude entwickelt API-001 parallel; Codex übernimmt unabhängige Gates, Integration und sämtliche Deployments. Der Produktionswechsel auf v16/Tabler ist erfolgt. Vorhandene Lanes sind terminal und inventarisiert. Recovery-Commits `5a5f98b` (A1: sachfremden TRUNCATE entfernt) und `6bc1cc7` (D: Schema-17-Testannahme) sind vorbereitet. D-Gate: `153 passed in 80.18s (0:01:20)`, `GATE_EXIT=0`; A1-Datenbankgate und unabhängige Integrationsgates bleiben offen und erfolgen nach Claudes Übergabe.
...
1. Claude vervollständigt Welle 1 und Welle 2 in `integrate/api-mcp-fhir-0905`: Schema/Schlüssel-Store D, REST A1, FHIR B, MCP, Swagger, Dokumentation, Admin-Verwaltung und schlüsselgeschützte Endpunkte. Die Recovery-Commits bleiben Bestandteil der Lieferung; Codex verändert die reservierten API-Worktrees nicht.
2. Claude integriert den aktuellen `main`-Stand, ergänzt den Sidebar-Eintrag mit Guard, aktualisiert das Manifest und meldet «BEREIT ZUM DEPLOY» mit Abschlusslogs, Commits und kombiniertem Gate. Fehlende DB-Umgebung oder übersprungene DB-Tests sind kein grüner Nachweis.
3. Codex liest danach den vollständigen Diff und wiederholt unabhängig Gesamtsuite, Ruff, Schema-, Swagger-Asset- und Offline-Paketprüfung; Security-Review und Cross-Vendor-Review folgen. **Den alten `gate.sh`-Aufruf nicht blind ausführen:** Er verweist auf eine gelöschte Datenbank. Aktuelle Wrapper und eine verfügbare isolierte Testdatenbank verwenden. Swagger-Verifikation aus Koordination §2.6 klären.
