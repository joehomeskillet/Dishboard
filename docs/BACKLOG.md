...
## Nachtrag — 18. September 2026: Geteilte Gänge (Schema 43) — offene Punkte aus der Integration

Folgeposten aus der Integration «geteilte Gänge, Schema 43»; keine Fertigmeldung der Lieferung.

1. **Signage-Kopfzeilen schneiden lange Gangtitel ab.** Befund: Auf dem Patienten-Wochenboard (1920×1080) sitzt die Gangzeile im Ausgabe-Band (`meal-name`, `text-nowrap overflow-hidden`) mit rund 7 px Reserve beim Demo-Datensatz («Suppe: Gemüsesuppe · Dessert: Fruchtsalat»); Rezepttitel sind längenmässig unbegrenzt, längere Namen werden abgeschnitten samt nachfolgender Allergensymbole. Risiko: Unvollständige Pflichtangaben auf Signage. Nächster Schritt: Längenbudget im Gangeditor oder Umbruchstrategie fürs Band; Browser-Test mit Maximaltitel.
2. **`database/validate_schema.py` — Live-Entscheidungscheck `patient_key_is_forbidden`.** Befund: Prüft nur `accompaniment_*` und Kostenfälle. Risiko: Neue erlaubte Schlüssel (`soup`, `nutrition`, `fat`) könnten fälschlich blockiert werden, ohne dass der Validator es meldet (Cross-Vendor-Review, LOW). Nächster Schritt: Repräsentative neue Schlüssel mit Erwartung «erlaubt» ergänzen.
3. **Kein automatischer Vollvergleich der Patient-Compact-Key-Listen.** Befund: `PATIENT_ALLOWED_COMPACT_KEYS` (`cafeteria/patient_payload.py`) und `ALLOWED_PATIENT_COMPACT_KEYS` (`database/validate_schema.py`) sind heute deckungsgleich, Drift bleibt ungetestet. Risiko: Stille Abweichung zwischen Runtime und Schema-Validator (Cross-Vendor-Review, LOW). Nächster Schritt: Assertion ergänzen.
4. **`_flags_from_snapshot` in `cafeteria/course_store.py`.** Befund: Defektes Snapshot-JSON wird über `current_app.logger` geloggt; ausserhalb eines App-Kontexts würde der Log-Aufruf selbst fehlschlagen. Risiko: Diagnose bricht in CLI-/Test-Pfaden statt des eigentlichen JSON-Fehlers ab. Nächster Schritt: Logger ohne App-Kontext-Abhängigkeit prüfen.
5. **`main` unabhängig von diesem Release rot.** Befund: 181 deterministische Fehlschläge im Volllauf (7832 Tests), u. a. 51× `test_tabler_package_verification`, 12× `test_print_template_archive_browser`, 6× `test_operations_scoped_csrf`, 6× `test_api_key_record`, 2× `test_capture_ops_live_proof` (`macro 'icon' takes no keyword argument 'class'` in `templates/admin/operations.html:66`), `test_schema_baseline_matches_sequential_migration_structure`. Risiko: Gates interpretieren «grün» fälschlich als «0 failed». Nächster Schritt: Eigene Aufräumwelle; bis dahin Gates gegen die Namensliste vergleichen, nicht gegen «0 failed».
6. **`MANIFEST_SHA256.txt`.** Befund: 23 Einträge trugen schon vor diesem Release falsche Prüfsummen (u. a. `cafeteria/admin/shopping_list_routes.py`, `static/admin-tabler.css`, `cafeteria/cost_calc.py`); zudem zieht ein voller `tools/build_manifest.py`-Lauf rund 1130 ungetrackte `.claude/**`-Dateien in die Paketliste. Risiko: Paketgate und Deploy-Integrität nicht verlässlich. Nächster Schritt: Manifest-Werkzeug auf getrackte Dateien begrenzen, Drift einmalig bereinigen.
7. **Test-Infrastruktur.** Befund: `pytest-randomly` würfelt die Reihenfolge je Lauf (Läufe nur über Test-IDs vergleichen); Test-Postgres-Container veröffentlichen ephemere Host-Ports, nach Neustart zeigen die `worker-*.env` auf tote Ports; Browser-/Capture-Tests überschreiben getrackte PNGs unter `.claude/evidence/**`. Risiko: Flaky oder irreproduzierbare Gates und versehentliche Evidence-Drift. Nächster Schritt: Wrapper mit `docker port`-Auflösung; Capture-Promotion von getrackten Baselines trennen.
8. **`or-agent` (OpenRouter) für Gate-WPs.** Befund: Shell-Tool kappt bei 300 s; Default-Modell beendete den Lauf nach dem Lesen ohne Edit. Risiko: Gate-Arbeitspakete bleiben unbearbeitet trotz grünem Start. Nächster Schritt: Für Gate-WPs andere Lane oder Laufzeit-/Modellvorgabe; nicht als alleiniger Executor.
...
**Historische Live-Nachweise zu `2cfc43f`:** Admin **738 Checks/36 Screenshots**, OPS **56 Checks/2 Screenshots**, ergänzend Vorlagenkatalog, Screens und Darstellung **193 Checks/10 Screenshots**, jeweils ohne fehlgeschlagene Checks. Datierte PDFs für 2026-08-31: 10 Cafeteria-/28 Patientenmenüs mit Fotos, je eine A4-Seite; Karten und Überlauf wurden in mehreren Browserbreiten geprüft. Die damaligen Screens-Vorschauen lieferten wegen fehlender Wochenpublikation korrekt HTML-404 mit `no-store`; das belegt Struktur und Fehlerzustand, keine gefüllten Vorschauen oder vollständige UI-Abnahme. Fachliche Druck-, Daten- und physische Playerabnahmen bleiben offen.
...
**Vorbereitet, nicht deployed:** `5b746b35d261ed8b552724b3b273e71fd393973c` enthält geprüfte Menü-Iconaktionen und deklarierte VEG/VGN-Angaben für UI-003/ICO-001/002. Kombinierte 121 Tests und Roots unabhängige Imageprüfung bestanden. Die vollständigen Releasegate-Versuche 6043 und 89103 endeten beide mit Exit 143, ohne JUnit oder Abschlussausgabe; die Ursache bleibt unbelegt. Root bewahrte beide Evidenzstände, bestätigte jeweils das Ende der Exportprozesse und eskalierte die Abbrüche. Kein dritter unveränderter Lauf und kein vollständiges PASS. Image `menuplan-symbols-0907:5b746b3-u022` wurde anhand von 677 Dateien ohne Abweichungen oder Extras, 247 AST-Prüfungen, Revision-Label, Benutzer, pip und Schema-20-Hashes unabhängig bestätigt (Build-WP `wp-1b31436a1455`). Das ist keine Produktivabnahme.

**Integrierte Folgearbeiten, nicht deployed:** Aktuelles Public-404 mit `no_published_menu` ist bei fehlender aktueller Publikation vertraglich erwartet. Der Tabler-Fix der noch produktiv verwendeten alten Leerseite `de80b7e` bestand Rootgate 64757 mit **1959 passed in 65.04s**, Ruff und den unveränderten Mypy-Basisvergleich. Er ist mit den geprüften Operator- und Dokumentationsänderungen in `37ddf54` integriert; Roots kombinierter Lauf bestand mit **2126 Tests in 132.06s**. Manifest, vollständige Releaseabnahme und Deploy dieses Integrationsstands stehen aus.
...
**Operative Reihenfolge:** OPS, Symbole, Public-/Operator-Folgearbeiten, B2/V1 und B3/V2 sind mit `783fab3` produktiv. Offene Live-Datenfälle bleiben abzunehmen; R1-B/Schema 22 benötigt nach dem Lizenzbefund-Fix die unabhängige Gesamtfreigabe. Danach bleiben offene Screens-/Vorlagen-/Branding-Funktionen, Grundlagen und Rezeptverwaltung, Suche/Importe/Produktdaten, Rezeptplanung/Einkauf/Druck/KI, Kalkulation/Lager/Bestellungen und weitere Anbindungen. Die Phasen unten erhalten alle Detailaufträge; Qualitäts- und Fachabnahmen begleiten jede Lieferung. Die historischen Symbol-Abbrüche bleiben ohne nachträglich erfundenes PASS erhalten.
...
| Produktiver Kern mit weiterem Ausbau | 1 | BAS-001: B1 und B2 mit Fix `7ca5286` sind in `2cfc43f`/Schema 21 produktiv. B3 ist seit `783fab3` ebenfalls produktiv; Lager-/Bestandsumfang bleibt offen. Keine vollständige BAS-Fertigmeldung. |
