...
from flask import Response, abort, current_app, flash, make_response, redirect, render_template, request, url_for
...
def complete_choices(reader, *args) -> list:
    result = []
    offset = 0
    while True:
...
def choices(row=None) -> dict[str, Any]:
...
def get_row(kind: str, public_id: str):
    if kind == 'zutaten':
...
def render_detail(kind: str, row=None, *, error=None, purpose=None):
...
def master_data_list():
    ...
    filters = {key: value for key, value in request.args.items() if key != 'page'}
    allowed = capabilities()
...
def master_data_new(kind: str):
    ...
    try:
        actor, _ = forms.expectations(kind, 'neu', None)
    except forms.LocationConflict:
        return render_location_conflict(kind, 'neu', None)
    try:
        values = forms.payload(kind, 'neu', None, set())
        if kind == 'zutaten':
            result = store.create_food(forms.engine(), actor, {**values, 'source_kind': 'manual'})
        elif kind == 'einheiten':
            result = store.create_unit(forms.engine(), actor, **values)
        else:
            result = store.create_vocabulary(forms.engine(), forms.VOCABULARY[kind], actor, **values)
...
@protected
def master_data_detail(kind: str, public_id: str):
    check_kind(kind)
    if request.args:
        abort(400)
...
@protected
def master_data_change(kind: str, public_id: str, purpose: str):
    check_kind(kind)
    ...
    if request.args:
        abort(400)
    try:
        actor, target = forms.expectations(kind, purpose, public_id)
    except forms.LocationConflict:
    ...
    try:
        ...
        if purpose in ('archivieren', 'reaktivieren'):
            ...
            if kind == 'zutaten':
                store.set_food_active(forms.engine(), actor, target, active=active)
            elif kind == 'einheiten':
                store.set_unit_active(forms.engine(), actor, target, active=active)
            else:
                store.set_vocabulary_active(forms.engine(), forms.VOCABULARY[kind], actor, target, active=active)
        elif kind == 'einheiten':
            store.rename_unit(forms.engine(), actor, target, **values)
        elif kind in forms.VOCABULARY:
            store.update_vocabulary(forms.engine(), forms.VOCABULARY[kind], actor, target, **values)
        elif purpose == 'stammdaten':
            store.update_food(forms.engine(), actor, target, values)
        elif purpose == 'tags':
            store.replace_food_tags(forms.engine(), actor, target, values['tag_public_ids'])
        elif purpose == 'metadaten':
            store.replace_food_metadata(forms.engine(), actor, target, **values)
        else:
            store.set_food_allergen_review(forms.engine(), actor, target, **values)
    except (MasterDataValidationError, MasterDataConflictError) as error:
