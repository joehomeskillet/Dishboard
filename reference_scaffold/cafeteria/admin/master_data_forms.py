...
from ..master_data_types import MasterDataValidationError, ObjectExpectation
from ..security import csrf_token, validate_csrf


KINDS = {'zutaten': 'Zutaten', 'einheiten': 'Einheiten', 'kategorien': 'Kategorien', 'tags': 'Tags'}
QUERY_KINDS = {'zutaten': 'foods', 'einheiten': 'units', 'kategorien': 'categories', 'tags': 'tags'}
VOCABULARY = {'kategorien': 'food_category', 'tags': 'tag'}
...
def expectations(kind: str, purpose: str, public_id: str | None):
    ...
    return ActorExpectation(original['actor'], original['actor_version']), target
...
def payload(kind: str, purpose: str, public_id: str | None, allergen_codes: set[str]) -> dict[str, Any]:
    ...
    if purpose in ('archivieren', 'reaktivieren'):
        ...
        fields = {'name'} | ({'code'} if not public_id else set()) | ({'sort_order'} if kind == 'kategorien' else set())
    if set(request.form) - base - fields - multiple or any(
