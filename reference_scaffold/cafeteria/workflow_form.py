...
from .workflow import MENU_TYPES, PROFILE_DAYS, PROFILE_MEALS, WorkflowValidationError
    ...
    if hasattr(form, 'getlist'):
        for key in form:
            if len(form.getlist(key)) != 1:  # type: ignore[attr-defined]
                raise WorkflowValidationError(f'Formularfeld mehrfach gesendet: {key}')
...
def _positive_integer(value: str, label: str) -> int:
    try:
        ...
        raise WorkflowValidationError(f'{label} muss eine ganze Zahl sein.') from error
    if parsed <= 0:
        raise WorkflowValidationError(f'{label} muss positiv sein.')
    return parsed
...
def parse_draft_form(profile_code: str, form: Mapping[str, str]) -> ParsedDraft:
    ...
    if missing:
        raise WorkflowValidationError(f'Fehlendes Formularfeld: {sorted(missing)[0]}')
    try:
        week_start = date.fromisoformat(supplied['week_start'])
    except ValueError as error:
        raise WorkflowValidationError('Wochenbeginn ist ungültig.') from error
    if week_start.isoweekday() != 1:
        raise WorkflowValidationError('Wochenbeginn muss ein Montag sein.')
    row_version = _positive_integer(supplied['row_version'], 'Versionsnummer')
    days = []
    for day_index in range(PROFILE_DAYS[profile_code]):
        services = []
        for meal_code in PROFILE_MEALS[profile_code]:
            service_prefix = f'service_{day_index}_{meal_code}'
            service_state = supplied[f'{service_prefix}_state']
            options = []
            for type_code in MENU_TYPES:
                option_prefix = f'{service_prefix}_{type_code}'
                option: dict[str, Any] = {
                    'type_code': type_code,
                    'title': supplied[f'{option_prefix}_title'].strip(),
                    'components': [
                        line.strip()
                        for line in supplied[f'{option_prefix}_components'].splitlines()
                        if line.strip()
                    ],
                }
                if profile_code == 'staff_guest':
                    if service_state == 'open':
                        option['internal_rappen'] = _positive_integer(
                            supplied[f'{option_prefix}_internal_rappen'],
                            'Mitarbeitendenbetrag',
                        )
                        option['external_rappen'] = _positive_integer(
                            supplied[f'{option_prefix}_external_rappen'],
                            'Gästebetrag',
                        )
                    else:
            ...
            services.append(
                {
                    ...
                    'notice': supplied[f'{service_prefix}_notice'].strip(),
                    'options': options,
                }
            )
        days.append(
