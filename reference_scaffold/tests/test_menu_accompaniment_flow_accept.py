...
def _login_as(app: Flask, engine: Engine, role: str, identity: str, *, csrf: str = 'accept-csrf'):
    suffix = {'editor': '101', 'publisher': '102', 'reader': '103'}[identity]
    actor = database.upsert_entra_user(
        engine,
        {
            ...
            'name': {'editor': 'Redaktion', 'publisher': 'Publikation', 'reader': 'Leser'}[identity],
...
def _add_session(context, client, base_url: str) -> None:
    ...
    context.add_cookies([{
        'name': 'session', 'value': cookie.value, 'url': base_url, 'httpOnly': True,
    }])
    ...
    for offset, day in enumerate(values['days']):
        ...
        for service in day['services']:
            for option in service['options']:
                option['title'] = (
                    'Kartoffelgratin' if option['type_code'] == 'MENU_1' else 'Gemüseteller'
                )
    return values
...
def test_editor_to_publish_all_consumers_and_legacy_week(
    ...
    with browser.new_context(
        ...
        for family, selected, plain in (
            ...
            assert page.locator('form[data-menu-editor], form[action$="/publish"]').count() == 0
            assert page.goto(f'/admin/{family}/menues').status == 200
            selected_row = page.locator('#menu-list tbody tr').filter(has_text=selected)
            plain_row = page.locator('#menu-list tbody tr').filter(has_text=plain)
            expect(selected_row.locator('.menu-accompaniment')).to_have_text('Dazu: Suppe')
            expect(plain_row.locator('.menu-accompaniment')).to_have_count(0)
    ...
    assert (plain['accompaniment_code'], plain['accompaniment_name']) == ('none', '')
    ...
    with httpx.Client(base_url=live_server, headers=headers, timeout=10.0) as http_client:
        ...
        mcp_week = invoke_tool(mcp, 'get_week_menu', {'channel': 'patienten'})
        mcp_preview = invoke_tool(
            mcp, 'get_week_preview', {'channel': 'patienten', 'week_start': DAY}
        )
        ...
        assert invoke_tool(mcp, 'list_weeks', {'channel': 'patienten'}) == weeks.get_json()
...
def _shift_csv_week(payload: bytes, days: int) -> bytes:
    ...
    for row in rows:
        row['datum'] = (dt.date.fromisoformat(row['datum']) + dt.timedelta(days=days)).isoformat()
