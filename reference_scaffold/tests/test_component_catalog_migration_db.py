...
def test_v13_real_postgres_contract_covers_catalog_constraints_and_component_links(
    ...
    assert database.SCHEMA_VERSION == version == 21
    assert database.APPLICATION_VERSION == 'dishboard-schema-v21'
    assert (plan[-1].version, plan[-1].path.name) == (21, '0018_v20_to_v21.sql')
...
def test_v15_migration_installs_role_scoped_active_location_lock(pg16: Engine) -> None:
    _run_v12_migrations(pg16)
    for version in (13, 14, 15, 16, 17, 18, 19, 20, 21):
        migration = next(
