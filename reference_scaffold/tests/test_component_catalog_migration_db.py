...
def test_v13_real_postgres_contract_covers_catalog_constraints_and_component_links(
    ...
    assert database.SCHEMA_VERSION == version == 20
    assert database.APPLICATION_VERSION == 'dishboard-schema-v20'
    assert (plan[-1].version, plan[-1].path.name) == (20, '0017_v19_to_v20.sql')
...
def test_v15_migration_installs_role_scoped_active_location_lock(pg16: Engine) -> None:
    _run_v12_migrations(pg16)
    for version in (13, 14, 15, 16, 17, 18, 19, 20):
        migration = next(
