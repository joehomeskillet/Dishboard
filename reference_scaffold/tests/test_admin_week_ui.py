...
def test_week_status_and_native_actions_are_visible_and_remain_available(
    ...
    expect(publish_trigger).to_have_text('Veröffentlichen')
    expect(page.locator('main .btn-primary:visible')).to_have_count(1)
    if state == 'review_open':
        expect(page.locator('main .btn-primary')).to_have_text('Offene Punkte prüfen')
    if state == 'ready':
    ...
    if state == 'ready':
        ...
        apply = page.get_by_role('button', name='Wochenvorgaben übernehmen', exact=True)
