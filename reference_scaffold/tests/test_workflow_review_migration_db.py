...
def test_v15_upgrade_preserves_checked_work_and_publication_without_fabricating_receipts(pg16):
    ...
    assert [entry.version for entry in applied] == list(range(4, 20))
    ...
    with pg16.connect() as connection:
        ...
        row = connection.execute(text('SELECT name,application_version,checksum_sha256 FROM cafeteria.schema_migrations WHERE version=19')).one()
        assert row == ('0016_v18_to_v19.sql', 'dishboard-schema-v19', hashlib.sha256(plan[-1].path.read_bytes()).hexdigest())
