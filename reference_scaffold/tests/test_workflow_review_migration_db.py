...
def test_v15_upgrade_preserves_checked_work_and_publication_without_fabricating_receipts(pg16):
    ...
    assert [entry.version for entry in applied] == list(range(4, database.SCHEMA_VERSION + 1))
    database._execute_script(pg16, str(PERMISSIONS))
    with pg16.connect() as connection:
        assert connection.execute(text(
            "SELECT to_jsonb(i)-'accompaniment' FROM cafeteria.menu_items i ORDER BY id"
        )).scalars().all() == items
        ...
        assert row == ('0017_v19_to_v20.sql', database.APPLICATION_VERSION,
