...
def test_filter_controls_combine_reset_and_keep_exact_results(request, family, profile, width, tmp_path):
    ...
    expect(summary).to_contain_text('Herkunft: Schweiz')
    ...
    page.locator('.component-filter-advanced summary').click()
    expect(summary).to_contain_text('Herkunft: Deutschland')
    expect(summary).to_be_visible()
    ...
    page.locator('.component-filter-advanced summary').click()
    expect(summary).to_contain_text('Herkunft: nicht erfasst')
    expect(summary).to_be_visible()
    form.get_by_role('link', name='Zurücksetzen', exact=True).click()
