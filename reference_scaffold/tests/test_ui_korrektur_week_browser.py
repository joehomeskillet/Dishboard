...
def test_a06_live_checked_and_missing_allergens_are_separate(
    ...
    expect(status).to_contain_text('Veröffentlicht · entspricht dem gespeicherten Stand')
    filled = SLOT_COUNT[family]
    expect(status).to_contain_text(f'Alle {filled} erfassten Menükarten geprüft')
    expect(status).to_contain_text('1 Menükarte ohne Allergenangaben')
