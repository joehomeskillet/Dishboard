...
def test_compact_default_without_local_control_preserves_help_and_targets(
    ...
    with _context(browser, live_server, client, javascript=javascript) as context:
        ...
        for family in ('cafeteria', 'patienten'):
            ...
            components_help = page.locator('.admin-hint > summary[aria-describedby="components-hint"]')
            hint = page.locator('#components-hint')
            expect(hint).to_be_hidden()
            components_help.focus()
            expect(components_help).to_be_focused()
            components_help.press('Enter')
            expect(hint).to_be_visible()
            expect(hint).to_contain_text('Aus Liste auswählen oder als eigenen Text erfassen — nicht beides gleichzeitig.')
            expect(hint).to_contain_text('Die Reihenfolge entspricht der Anzeige.')
            components_help.press('Enter')
            expect(hint).to_be_hidden()
            ...
            for row in page.locator('.menu-editor-component-row').all():
                component = row.locator('select[name="component_public_id"]')
                text = row.locator('input[name="component_text"]')
                expect(component).to_have_attribute('aria-describedby', 'components-hint')
