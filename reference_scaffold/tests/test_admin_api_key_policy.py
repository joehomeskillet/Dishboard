...
def test_no_js_creation_channels_and_revoke_flow(admin_client, browser, width):
    ...
    try:
        with browser.new_context(java_script_enabled=False, reduced_motion='reduce',
            ...
            expect(revoke.locator('summary')).to_have_class(re.compile(r'\bbtn-danger\b'))
            expect(revoke.locator('summary')).to_have_attribute('title', re.compile(r'widerrufen', re.I))
            expect(revoke.get_by_role('button', name='Bestätigen', exact=True)).not_to_be_visible()
            more = row.locator('.ui-sem-actions > summary')
            expect(more).to_have_attribute('title', re.compile('.'))
            more.click()
            details = row.locator('details.admin-api-key-details > summary')
