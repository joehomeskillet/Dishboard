...
def test_browser_select_save_reload_and_no_latest_swap(
    ...
    page.goto(EDITOR)
    _ready(page)
    page.get_by_label('Menüname', exact=True).fill('Gebundenes Menü')
    page.locator('[data-edit-row]').first.click()
    page.locator('[data-component-kind-option][value="text"]').first.check()
    page.locator('[name="component_text"]').fill('Browser-Suppe')
...
def test_browser_detach_confirm_is_not_a_side_effect(
    ...
    page.goto(EDITOR)
    page.get_by_label('Menüname', exact=True).fill('Mit Bindung')
    page.locator('[data-edit-row]').first.click()
    page.locator('[data-component-kind-option][value="text"]').first.check()
    page.locator('[name="component_text"]').fill('Lösesuppe')
    ...
    page.get_by_label('Menüname', exact=True).fill('Anderer Titel')
    page.locator('[data-edit-row]').first.click()
    expect(page.get_by_label('Rezeptrevision')).to_have_value(revision['public_id'])
...
def test_nojs_native_select_search_absent_and_archived_readable(
    ...
    page.goto(EDITOR)
    _ready(page)
    page.get_by_label('Menüname', exact=True).fill('Ohne JavaScript')
    page.locator('[name="component_text"]').fill('Archivsuppe')
    page.get_by_label('Rezeptrevision').select_option(revision['public_id'])
...
def test_recipe_selector_viewports_keyboard_zoom_and_fonts(
    ...
    _ready(page)
    page.get_by_label('Menüname', exact=True).fill(f'Viewport {width}')
    page.locator('[data-edit-row]').first.click()
    page.locator('[data-component-kind-option][value="text"]').first.check()
    page.locator('[name="component_text"]').fill(f'Sicht-{width}')
