...
31. `0031_v33_to_v34.sql` (34): persistierte Einkaufslisten (`shopping_lists` mit unveränderlichem Standort, unveränderliche `shopping_list_revisions` mit erzwungener Snapshot-Hülle und Hash, `shopping_list_manual_items`, listengebundener `shopping_list_line_status`); Texte per CHECK in getrimmter Normalform, reine Tabellenrechte, kein neues Definer-Verb. Ein App-Rollback auf v33 läuft mit dem Repo-Compose nur über `APP_IMAGE=<v33-Digest> docker compose up -d --wait --no-deps app`, weil `migrate` mit v33-Code an Version 34 abbricht und `app` davon abhängt; Einkaufslisten bleiben dabei erhalten und unsichtbar, ein Schema-Rollback ist nur per Restore möglich.
...
R1 enthält keine Rezeptoberfläche, Menübindung oder Importanbieter. Nach Rezeptwrites ist ein Schema21-Client kein bestätigter Rückfall: Daten und Revisionen erhalten und einen kompatiblen Forward-Fix verwenden; keine Rezepttabellen löschen und keine alten Nutz-/Authdaten automatisch zurückspielen. Root prüft vor Release vollständiges Paket, Backup, Migration und Livebelege.

Schema v21 liefert B2/M-A: zehn Stammdatentabellen, neun Einheiten mit geschützter Semantik, standortgebundene Zutaten/Vokabulare/Lagerzuordnungen und getrennte unveränderliche Vorschlagsquellen und Entscheidungen. Alle Fachwrites laufen über explizite Definer-Verben mit ursprünglichen Actor-/Objektversionen, fester Sperrreihenfolge und atomarem Audit. Identische normalisierte Speicherung verändert keine Version oder Zeitstempel; Archivieren eines zugeordneten Lagerorts wird verweigert. Übernahmen erzeugen keine Zutat und überschreiben keine bestätigten Werte. `cafeteria_app` erhält SELECT und ausschließlich die öffentlichen Verben, der Auth-Issuer keine neuen Rechte. B3/B4/R1 verwenden nur `master_data_store` und `master_data_types`; keine zweite DML-Schicht. Food-Anlage/-Bearbeitung nimmt `name`, `base_unit_code`, optional `category_public_id`, `density_g_per_ml`, `piece_weight_g`, `note`; nur Anlage zusätzlich die vereinbarten `source_*`-Felder und `fetched_at`. Metadaten, Tags, Lagerorte und Prüfstatus haben eigene Aggregataktionen. Keine Menübindung, Rezepttabellen oder Bestandsbuchung und keine Änderung an Publikationsbytes. Vor produktivem Schemawechsel ist ein geprüftes Backup erforderlich; danach nur datenbewahrende Forward-Fixes.

Schema v20 ergänzt Betriebszeiten und geschützte Bereichsnamen. `menu_services` erhält die nullbaren Wanduhrzeiten `service_start` und `service_end` (Zeitzone aus `locations.timezone`, Europe/Zurich) mit dem benannten Constraint `menu_services_time_window_check`; Nachtfenster mit Ende ≤ Beginn sind verboten, bestehende Zeilen bleiben ohne Zeiten. `offer_profiles.display_name` ist über `offer_profiles_display_name_check` auf nicht leeren Text bis 80 Zeichen begrenzt; `cafeteria_app` darf nur `display_name` und `allows_weekend` schreiben; Profilcodes, Preisflag und Mahlzeiten bleiben geschützt. Der benannte `offer_profiles_profile_contract_check` hält Patientenwochenenden fest offen und erlaubt den Cafeteria-Wochenendschalter. Neue Cafeteria-Wochenenddienste benötigen die Freigabe; vorhandene Zeilen bleiben bei unverändertem Slot auch nach Abschaltung aktualisierbar. `validate_menu_item_price` prüft deshalb nur noch Profil und Mahlzeit (Kosten ausschliesslich im Cafeteria-Mittag): Der Serviceschreiber entscheidet datumsgenau über die Wochenendfreigabe, sodass freigegebene Samstags- und Sonntagsmenüs reguläre Preise tragen und nach dem Abschalten des Schalters bepreisbar bleiben. Wiederkehrende Wochenvorgaben liegen als JSONB unter dem bestehenden Settings-Schlüssel `operations_schedule` je Standort und Profil; Speichern ist ein Compare-and-Set über `revision` mit ursprünglicher `authz_version` und aktivem Adminnachweis unter bis zum Commit gehaltenen IAM-Rollen-/Actorsperren (`lock_operations_actor`, READ COMMITTED). Nur Erwartung 0 darf anlegen; höhere Erwartungen aktualisieren ausschliesslich passende vorhandene Zeilen. Der Seed überschreibt gepflegte `offer_profiles.display_name` und `meal_periods.display_name` nicht mehr und erhält auch den gepflegten Cafeteria-Wochenendschalter; feste Preis-/Mahlzeitenregeln beziehungsweise `sort_order` werden weiter durchgesetzt. `workflow_week_context` liefert Servicezeiten als `start`/`end` **innerhalb** von `jsonb_strip_nulls`, sodass Wochen ohne Zeiten byteidentisches Kontext-JSON und damit gültige Prüfbelege behalten. Publikationssnapshots dürfen als Schema 2 optional `area_name` (nicht leer, höchstens 80 Zeichen) und je Service `service_start`/`service_end` (Muster `HH:MM`, Ende nach Beginn) führen; `patient_key_is_forbidden` erlaubt dafür zusätzlich die Schlüssel `servicestart`, `serviceend` und `areaname`. Fehlende Schlüssel bleiben gültig: Schema-1-Snapshots werden nicht umgeschrieben und weiterhin akzeptiert. Cafeteria-Snapshots erlauben Sa/So jeweils null oder einen LUNCH-Service; die Gültigkeit publizierter Wochen hängt nicht vom aktuellen Schalter ab. Die Migration verändert keine bestehenden Servicezeilen und keine Snapshotbytes.
