...
Schema v20 ergänzt Betriebszeiten und geschützte Bereichsnamen. `menu_services` erhält die nullbaren Wanduhrzeiten `service_start` und `service_end` (Zeitzone aus `locations.timezone`, Europe/Zurich) mit dem benannten Constraint `menu_services_time_window_check`; Nachtfenster mit Ende ≤ Beginn sind verboten, bestehende Zeilen bleiben ohne Zeiten. `offer_profiles.display_name` ist über `offer_profiles_display_name_check` auf nicht leeren Text bis 80 Zeichen begrenzt; `cafeteria_app` darf ausschliesslich diese eine Spalte schreiben (`GRANT UPDATE (display_name)`), Profilcodes, Preis- und Rasterflags bleiben ohne Schreibrecht. Wiederkehrende Wochenvorgaben liegen als JSONB unter dem bestehenden Settings-Schlüssel `operations_schedule` je Standort und Profil; Speichern ist ein Compare-and-Set über `revision` mit Akteur-, `authz_version`- und Adminnachweis in einer Anweisung. Der Seed überschreibt gepflegte `offer_profiles.display_name` und `meal_periods.display_name` nicht mehr und setzt nur noch die harten Rasterflags beziehungsweise `sort_order` durch. `workflow_week_context` liefert Servicezeiten als `start`/`end` **innerhalb** von `jsonb_strip_nulls`, sodass Wochen ohne Zeiten byteidentisches Kontext-JSON und damit gültige Prüfbelege behalten. Publikationssnapshots dürfen als Schema 2 optional `area_name` (nicht leer, höchstens 80 Zeichen) und je Service `service_start`/`service_end` (Muster `HH:MM`, Ende nach Beginn) führen; `patient_key_is_forbidden` erlaubt dafür zusätzlich die Schlüssel `servicestart`, `serviceend` und `areaname`. Fehlende Schlüssel bleiben gültig: Schema-1-Snapshots werden nicht umgeschrieben und weiterhin akzeptiert. Die Migration verändert keine bestehenden Servicezeilen und keine Snapshotbytes.
